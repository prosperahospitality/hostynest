...
export async function POST(req) {
  ...
  } else if (payload.operation === "editprice") {
    try {
      await db.connect();
      ...
      const updatePromises = updates.map(update => {
        const { id, rate_3hr, rate_6hr, rate_12hr, rate_24hr } = update;
        return Pms_Ratesandinventory_Managerateandinventory.findOneAndUpdate(
          { id: id },
          {
            rate_3hr: rate_3hr,
            rate_6hr: rate_6hr,
      ...
      const updatedHotels = await Promise.all(updatePromises);
      console.log(updatedHotels, "updatedHotels");

      const notFoundHotels = updatedHotels.filter(hotel => !hotel);
      if (notFoundHotels.length) {
        return NextResponse.json({ status: 404, message: `${notFoundHotels.length} hotels not found` });
      }
      ...
      return NextResponse.json({ status: 500, message: "An error occurred during update" });
    }
  } else {

...
export async function POST(req) {
  ...
  }else if (payload.action === "updateAvail") {
  ...
  return NextResponse.json({ success });
    ...
    const cleanedPayload = payload.map((item) => {
      const { _id, ...rest } = item; // Remove _id to prevent duplicate errors
      return rest;
    });

    await Pms_Propertymaster_Roomamenities.insertMany(cleanedPayload);

    const data = await Pms_Propertymaster_Roomamenities.find();

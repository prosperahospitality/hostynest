...
import { Button, Modal, ModalContent, ModalBody, ModalFooter, useDisclosure, Tabs, Tab, Card, CardBody, Input, Slider, RadioGroup, Radio, Switch } from "@nextui-org/react";
import { Save } from 'lucide-react'

const EditModal = ({ result, rowDataID, isSoldOut, onUpdateTable }) => {
    const [selected, setSelected] = useState("updateRate");
    ...
    const handleEditAction = async (rowDataID) => {
        ...
        let selec = result.find((item) => item.id === rowDataID);
        console.log("selec 000000: ", selec)
        setSelectedRow(selec)
    ...
    // function generateUniqueId(prefix = "MRI", start = 1, length = 5) {
    //     // Get the current timestamp to ensure uniqueness
    //     const timestamp = new Date().getTime();

    //     // Combine the timestamp and start value
    //     const uniqueNumber = start + timestamp % (10 ** length);

    //     // Pad the unique number with leading zeros
    //     const paddedNumber = String(uniqueNumber).padStart(length, "0");

    //     // Return the final unique ID
    ...
    const handleSave = async () => {
        ...
        if (selected === "updateArrival") {
            ...
            const payload = {
                ...
                first_checkin_last_checkout_status_3hr: isSelected3hr ? "Active" : "Inactive",
                first_checkin_last_checkout_status_6hr: isSelected6hr ? "Active" : "Inactive",
                first_checkin_last_checkout_status_12hr: isSelected12hr ? "Active" : "Inactive",
                creation_date: getCurrentDateTime(),
                last_update_on: getCurrentDateTime(),
                action: "updated",
            };

            const updateTable = result?.map((item) => {
        ...
        if (selected === "updateRate") {
            const payload = {
                ...selectedRow,
                rate_3hr: parseInt(hr3Rate),
                rate_6hr: parseInt(hr6Rate),
                rate_12hr: parseInt(hr12Rate),
                rate_24hr: parseInt(hr24Rate),
                total_rooms_count: parseInt(totalRooms),
                creation_date: getCurrentDateTime(),
                last_update_on: getCurrentDateTime(),
                action: "updated",
            };
                        ...
                        <>
                            <ModalBody>
                                <Tabs aria-label="Options" selectedKey={selected} onSelectionChange={setSelected}>
                                    <Tab key="updateRate" title="Update Rate & Rooms" className={`${isSoldOut ? 'opacity-50 pointer-events-none' : ''}`}>
                                        <Card>
                                            <CardBody>
                                                ...
                                                <div className='flex gap-1 items-center mt-2'>
                                                    <h4 className='text-base text-foreground-600 font-semibold'>Selected Day: </h4>
                                                    <div className='text-base text-foreground-600 font-semibold'>{selectedRow?.booking_date.split(" ").slice(0, 1).join(" ")}</div>
                                                </div>
                                    ...
                                    <Tab key="updateArrival" title="Update Check-In" className={`${isSoldOut ? 'opacity-50 pointer-events-none' : ''}`}>
                                        <Card>
                                            <CardBody>
                                                <div className='flex items-center'>
                                                    <h4 className='text-base text-foreground-600 font-semibold'>Selected Date: </h4>
                                    ...
                                    <Tab key="updateStatus" title="Update Sold Out">
                                        <Card>
                                            <CardBody>
                                                <div className='flex items-center'>
                                                    <h4 className='text-base text-foreground-600 font-semibold'>Selected Date: </h4>
